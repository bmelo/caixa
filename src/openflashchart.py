...
class graph:
	...
	def bar_outline( self, alpha, colour, outline_colour, text='', size=-1 ):
		tmp = '&filled_bar'
	...
	def pie( self, alpha, line_colour, label_colour, gradient=True, border_size=-1 ):
		
		self._pie = "%s,%s,%s" % (alpha,line_colour,label_colour)
		
		if( gradient ):
			self._pie += ",%s" %("true")

		if ( border_size > 0 ):
			self._pie += ",%s" %(border_size)
	...
	def pie_values(self, values, labels, links):
		self._pie_values = ','.join([str(v) for v in values]) 
		self.pie_labels = ','.join([str(v) for v in labels])	
		self.pie_links = ','.join([str(v) for v in links])	
