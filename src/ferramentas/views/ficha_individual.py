...
@login_required
def mostrar_ficha_individual(request):
    ...
    if request.GET.get('pdf'):
        return render_to_pdf('admin/includes_ferramentas/ficha_individual_pdf.html',locals())
    else:
        return render_to_response('admin/includes_ferramentas/ficha_individual.html',locals(),context_instance=RequestContext(request))

@login_required
def buscar_ficha_individual(request):
    title = 'Ficha Individual'
    year = datetime.date.today().year
    colegio = EscolaPrincipal.objects.filter(ativo=True)
    return render_to_response('admin/includes_ferramentas/buscar_ficha_individual.html',locals(),context_instance=RequestContext(request))
...
def get_alunos(request): 
    ...
    if request.GET.get('nome'): 
        ...
        alunos = []
    return render_to_response('admin/includes_ferramentas/ficha_individual_alunos.html',locals(),context_instance=RequestContext(request))
