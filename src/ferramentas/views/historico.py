...
def mostrar_historico(request):
    ...
    if request.GET.get('pdf'):
        return render_to_pdf('admin/includes_ferramentas/historico_pdf.html',locals())
    else:
        return render_to_response('admin/includes_ferramentas/historico.html',locals(),context_instance=RequestContext(request))
...
def buscar_historico(request):
    ...
    year = datetime.date.today().year
    colegio = EscolaPrincipal.objects.filter(ativo=True)
    return render_to_response('admin/includes_ferramentas/buscar_historico.html',locals(),context_instance=RequestContext(request))
...
def get_alunos(request): 
    ...
    if request.GET.get('nome'): 
        ...
        alunos = []
    return render_to_response('admin/includes_ferramentas/historico_alunos.html',locals(),context_instance=RequestContext(request))
